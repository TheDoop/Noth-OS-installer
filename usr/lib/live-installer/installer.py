...
import os, subprocess, time, shutil, gettext, stat, commands, sys, parted
...
class InstallerEngine:
    ...
    def step_format_partitions(self, setup):        
        for partition in setup.partitions:                    
            if(partition.format_as is not None and partition.format_as != ""):                
                ...
                self.update_progress(total=4, current=1, pulse=True, message=_("Disk Biçimleniyor %(partition)s - %(format)s..." % {'partition':partition.partition.path, 'format':partition.format_as}))
        ...
        try:
            ...
            for top,dirs,files in os.walk(SOURCE):
                ...
                for name in dirs + files:
                    ...
                    yuzde=(float(our_current)/float(our_total))*100
                    yuzde=float(int(yuzde*100))/100
                    self.update_progress(total=our_total, current=our_current, message=_("%" + str(yuzde) + " | " + str(our_current) + "/" + str(our_total) + " | Kopyalanıyor %s " % rpath))
            ...
            for dirtime in directory_times:
                ...
                try:
                    ...
                    self.update_progress(pulse=True, message=_("Meta bilgileri yükleniyor %s" % directory))
            ...
            os.system("cp -f /boot/vmlinuz* /target/boot/")
            os.system("cp /lib/live/mount/medium/live/initrd* /target/boot/")
            
            if (setup.gptonefi):
                os.system("mkdir -p /target/boot/efi/EFI/blink")
                os.system("cp -f /EFI/BOOT/grubx64.efi /target/boot/efi/EFI/blink")
            ...
            self.do_run_in_chroot('echo "%s\n \n \n \n y\n" | adduser -shell %s --disabled-password %s' % (setup.real_name,"/bin/bash", setup.username))
            ...
            # Add user's face
            os.system("cp /usr/share/pixmap/faces/t9penguino_trans.png /target/home/%s/.face" % setup.username)
            self.do_run_in_chroot("chown %s:%s /home/%s/.face" % (setup.username, setup.username, setup.username))
            
            #Delete live User
            self.do_run_in_chroot("deluser blink ; rm -f /etc/sddm.conf") 
            
            # Make the new user the default user in KDM            
            if os.path.exists('/target/etc/kde4/kdm/kdmrc'):
                defUsrCmd = "sed -i 's/^#DefaultUser=.*/DefaultUser=" + setup.username + "/g' " + "/target/etc/kde4/kdm/kdmrc"
                print defUsrCmd
                os.system(defUsrCmd)
            ...
            if(not setup.skip_mount):
                for partition in setup.partitions:
                    if (partition.mount_as is not None and partition.mount_as != "None"):
                        ...
                        if(fs == "swap"):
                            if setup.gptonefi:
                                print ("skip swap partition: %s" % partition_uuid)
                            else:
                                fstab.write("%s\tswap\tswap\tsw\t0\t0\n" % partition.partition.path)
                        else:
                            if (partition.mount_as == "/" ):
                                fstab.write("%s\t%s\t%s\t%s\t%s\t%s\n" % (partition.partition.path, partition.mount_as, fs, fstab_mount_options, "0", fstab_fsck_option))
            fstab.close()
    ...
    def finish_install(self, setup):
        try:
            ...
            hostnamefh.close()
            try:
                os.system("wget https://raw.githubusercontent.com/blinkcix/Guvenli_Internet/master/hosts hosts")
                os.system("mv -f hosts /target/etc/hosts")
            except:
            ...
            os.system("echo LANG=%s.UTF-8 > /target/etc/default/locale" % setup.language)
            os.system("echo LC_ALL=%s.UTF-8 > /target/etc/default/locale" % setup.language)
            self.do_run_in_chroot("update-locale LANG=%s.UTF-8" % setup.language)
            self.do_run_in_chroot("update-locale LC_ALL=%s.UTF-8" % setup.language)
            ...
            if(setup.grub_device is not None):
                ...
                grub_retries = 0
                self.do_check_grub(our_total, our_current)
                self.do_configure_grub(our_total, our_current)
            ...
            self.update_progress(pulse=True, total=our_total, current=our_current, message=_("Removing live-installer"))
            print " --> Remove Live-installer"
            self.do_run_in_chroot("apt-get remove -y blink-installer")
            self.do_run_in_chroot("apt-get install -y grub-pc-bin grub-efi")
            ...
            #Update system
            #self.update_progress(pulse=True, total=our_total, current=our_current, message=_("Updating system"))
            #print " --> system update"
            #self.do_run_in_chroot('if zenity --question --text "%s" ; then  xterm -e "apt-get update"  ; xterm -e "apt-get upgrade -y"; fi' %("Sistem Güncellensin mi? (İnternet Bağlantısı Gerektirir)"))

            #Set DNS
            self.update_progress(pulse=True, total=our_total, current=our_current, message=_("DNS Ayarlanıyor."))
            self.do_run_in_chroot('echo "nameserver 176.103.130.132 " > /etc/resolv.conf')
            self.do_run_in_chroot('echo "nameserver 176.103.130.134 " >> /etc/resolv.conf')
    ...
    def do_check_grub(self, our_total, our_current):
        ...
        if os.path.exists("/target/boot/grub/grub.cfg"):
            ...
            for line in grubfh:
                line = line.rstrip("\r\n")
                if("05_debian_theme" in line):
                    found_theme = True
                    print " --> Found Grub theme: %s " % line
                if ("menuentry" in line and "blink" in line):
                    found_entry = True
