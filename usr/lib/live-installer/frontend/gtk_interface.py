...
gettext.install("live-installer", "/usr/share/blink/locale")
...
class InstallerWindow:
    ...
    def __init__(self, fullscreen=False):
        ...
        self.wizard_pages[self.PAGE_INSTALL] = WizardPage(_("Installing noth..."), "install.png")
        ...
        self.wTree.get_widget("face_image").set_from_file("/usr/share/pixmaps/faces/t9penguino_trans.png")   
    ...
    def i18n(self):
        ...
        # hdd page
        self.wTree.get_widget("label_radio_hdd").set_label(_("Install blink on the selected drive:"))
    ...
    def edit_partitions(self, widget, data=None):
        ''' edit the partitions ... '''
        if os.path.isfile("/usr/bin/gparted-pkexec"):
            os.popen("gparted-pkexec &")
        else:
            os.popen("partitionmanager &")
...
.shine {position: relative;opacity: .2; top: 0;right: 0;bottom: 0;left: 0;background: #fff;-moz-border-radius: 0px;-webkit-border-radius: 0px;border-radius: 0px;margin:1px;} \
    ...
    def assign_language(self, treeview, data=None):
        ...
        self.setup.print_setup()
        try:            
            self.translation = gettext.translation('live-installer', "/usr/share/blink/locale", languages=[self.setup.language])
            self.translation.install()
        except Exception, detail:
            print "No translation found, switching back to English"
            self.translation = gettext.translation('live-installer', "/usr/share/blink/locale", languages=['en'])
            self.translation.install()        
        try:
            self.i18n()
        except:
        ...
        if(not goback):
            if(sel == self.PAGE_LANGUAGE):
                ...
                if not found_root_partition:
                    MessageDialog(_("Installation Tool"), _("<b>Please select a root partition</b>"), gtk.MESSAGE_ERROR, self.window, _("A root partition is needed to install blink.\n\n - Mount point: /\n - Recommended size: Larger than 10GB\n - Recommended format: ext4\n ")).show()
                    return
